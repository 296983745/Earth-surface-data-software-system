...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：Repetition -> testshengliudiqu
@IDE    ：PyCharm
@Author ：haomengqi
...
def get_relation(privince1, province2):
    ...
    if row[7].value == "相等":
        ...
        val = 0.167 + (0.167 / 32) * (32 - row[4].value)
        return "相接-" + str(format(val, '.3f'))
    elif row[7].value == "相交":
        val = 0.5+(0.167 / 32) * (32 -row[4].value)
        return "相交-" + str(format(val, '.3f'))
...
# 自定义列头
x = get_mysql_list('earthdata', 'data_copy', 'DataName')
...
# 保存 Excel 文件
wb.save('spacesimishengliudiqu.xlsx')
